...
async def match_archived():
    ...
    # set title field
    embed.add_field(name=f"Match Concluded!", value=f"Thanks for playing! :smile:")
    # set footer
    embed.set_footer(icon_url=footer_icon, text=f"{tourney_name} | {footer_note}")
...
async def purged():
    ...
    embed = discord.Embed(color=discord.Colour.green())
    # set title field
    embed.add_field(name=f"Purged!", value=f"All inactive tournament channels were purged.")
    # set footer
    embed.set_footer(icon_url=footer_icon, text=f"{tourney_name} | {footer_note}")
