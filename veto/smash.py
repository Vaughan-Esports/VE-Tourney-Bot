...
async def initial(ctx: discord.ext.commands.Context, bot: discord.ext.commands.Bot, main_message: discord.Message,
    ...
    # setup stages
    removed_stages = []
    removed_stages = removed_stages + counterpick_stages
    p1_dsr_stage = []
    p2_dsr_stage = []
...
async def nonInitial(ctx: discord.ext.commands.Context, bot: discord.ext.commands.Bot, main_message: discord.Message,
                     p1: discord.User, p2: discord.User, p1_dsr: list, p2_dsr: list,
                     emb: discord.Embed, game_num: int, starter_index: int, counter_index: int):
    """
    ...
    embed.set_field_at(counter_index, name="Counterpick Stages", value=counterpick_stages_message(removed_stages))
    await main_msg.edit(embed=embed)
    ...
    # switch player 1 to winner and player 2 to loser
    if msg.author == player2:
        player1, player2, p1_dsr_stage, p2_dsr_stage = await player_utils.swap_players(player1, player2, p1_dsr_stage,
                                                                                       p2_dsr_stage)
